...
*<h5>A unique answer is defined here as one that does not appear in any other NYT crossword puzzle in either the Shortz or pre-Shortz eras (either before or after the puzzle release date). The (moderate) strength of the all 15x15 puzzle day correlation is likely  related to the most difficult puzzle days (Fri and Sat) having prominent rightward shifts in the FDP. Saturday, which had the widest feature value range of the 15x15 puzzle days, had a positive correlation of approximately the same strength as the overall 15x15 correlation. Thus, while uniqueness is perhaps an overly stringent criterion to define answer rarity (see **Fig. 19**), there is still potential for this feature to have preditive value in the modeling phase.*
